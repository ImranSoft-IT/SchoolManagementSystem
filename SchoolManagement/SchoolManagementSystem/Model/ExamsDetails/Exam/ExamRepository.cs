...
namespace SchoolManagementSystem.Model
{
    public class ExamRepository : IExamRepository
    {
        ...
        public IEnumerable<Exam> GetExamtypeWiseAllExam(int branchId, int year, string examType)
        {
            var examLs = _context.Exam.Where(e => e.BranchId == branchId && e.StartDate.Year == year && e.ExamType == examType).ToList();
            if (examLs.Count() > 0)
            {
                return examLs;
            }
